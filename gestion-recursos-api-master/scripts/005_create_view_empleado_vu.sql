CREATE OR REPLACE FORCE VIEW "BITACORA2"."DF_EMPLEADO_VU"  ("ID", "CODEMPLEADO", "CODPERSONA", "NOMBRE", "CODTIPODOCUMENTO", "TIPODOCUMENTO", "NUMERODOCUMENTO", "NOMBRE1", "NOMBRE2", "APELLIDO1", "APELLIDO2", "TELEFONO", "CORREO", "DIRECCIONRESIDENCIA", "GENERO", "GENERONOMBRE", "FECHANACIMIENTO", "CODPERFIL", "PERFIL", "CODPERFILTIPO", "PERFILTIPO", "CODLINEAPRODUCTO", "LINEAPRODUCTO", "CODPROVEEDOR", "PROVEEDOR", "CODPERFILNIVEL", "PERFILNIVEL", "USUARIOCLARO", "FECHAINICIO", "FECHAFIN", "CODESTRUCTURAORGANIZACIONAL", "ROL", "CODMODALIDAD") AS 
  SELECT 	ROWNUM ID,
			E.ID CODEMPLEADO,
            P.ID CODPERSONA,
    ...
    LEFT JOIN DF_USUARIO U ON U.USUARIO = E.USUARIOCLARO
    LEFT JOIN DF_USUARIO_ROLES UR ON U.ID = UR.CODUSUARIO AND UR.CODUSUARIOROL <> 3;