...
import com.claro.gestionrecursosapi.reporte.domain.ReporteProrrateoGeneralService;
...
@RestController
@RequestMapping("/api/v1/reporte/prorrateo/general")
public class ReporteProrrateoGeneralController {

    @Autowired
    private ReporteProrrateoGeneralService service;
    ...
    public ResponseEntity<?> informe(@RequestHeader("CL_USUARIO") String CLUSUARIO, String q) {
        try {
            ...
            Iterable<ReporteProrrateoGeneral> resultado = service.generarReporte(CLUSUARIO, modelo);

            RespuestaCustomizada<Iterable<ReporteProrrateoGeneral>> respuesta = new RespuestaCustomizada<>();
    ...
    public HttpEntity<ByteArrayResource> download(String CLUSUARIO, String q) {
        try {
            ...
            header.set(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=prorrateo_general_" + dateFormat.format(fecha) + ".xlsx");
