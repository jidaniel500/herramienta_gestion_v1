...
import com.claro.gestionrecursosapi.reporte.domain.ReporteRecursoConProveedorService;
...
@RestController
@RequestMapping("/api/v1/reporte/proveedor")
public class ReporteRecursoConProveedorController {

    @Autowired
    private ReporteRecursoConProveedorService service;
    ...
    public ResponseEntity<?> informe(@RequestHeader("CL_USUARIO") String CLUSUARIO, String q) {
        try {
            ...
            Iterable<ReporteRecursoConProveedor> resultado = service.informe(CLUSUARIO, modelo);

            RespuestaCustomizada<Iterable<ReporteRecursoConProveedor>> respuesta = new RespuestaCustomizada<>();
    ...
    public HttpEntity<ByteArrayResource> download(String CLUSUARIO, String q) {
        try {
            ...
            header.set(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=recursos_proveedor_id_" + dateFormat.format(fecha) + ".xlsx");
