...
import com.claro.gestionrecursosapi.reporte.domain.ReporteListadoSolicitudesService;
...
@RestController
@RequestMapping("/api/v1/reporte/solicitudes")
public class ReporteListadoSolicitudesController {

    @Autowired
    private ReporteListadoSolicitudesService service;
    ...
    public ResponseEntity<?> informe(@RequestHeader("CL_USUARIO") String CLUSUARIO, String q) {
        try {
            ...
            Iterable<ReporteListadoSolicitudes> resultado = service.generarReporte(CLUSUARIO, modelo);

            RespuestaCustomizada<Iterable<ReporteListadoSolicitudes>> respuesta = new RespuestaCustomizada<>();
    ...
    public HttpEntity<ByteArrayResource> download(String CLUSUARIO, String q) {
        try {
            ...
            header.set(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=listado_solicitudes_" + dateFormat.format(fecha) + ".xlsx");
