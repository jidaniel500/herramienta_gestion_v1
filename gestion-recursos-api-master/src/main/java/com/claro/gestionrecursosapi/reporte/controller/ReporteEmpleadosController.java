...
import com.claro.gestionrecursosapi.reporte.domain.ReporteEmpleadosService;
import com.claro.gestionrecursosapi.reporte.domain.ReporteOpexService;
import com.claro.gestionrecursosapi.reporte.domain.ReporteRhPagosService;
...
@RestController
@RequestMapping("/api/v1/reporte/empleados")
public class ReporteEmpleadosController {

	@Autowired
	private ReporteEmpleadosService service;

	@GetMapping
	public ResponseEntity<?> informe(
			@RequestHeader("CL_USUARIO") String CLUSUARIO, String q) {
		try {
			q = URLDecoder.decode(q, StandardCharsets.UTF_8.toString());
			ObjectMapper mapper = new ObjectMapper();
			ReporteFiltroDto modelo = mapper.readValue(q,
					ReporteFiltroDto.class);

			Iterable<ReporteEmpleados> resultado = service.generarReporte(
					CLUSUARIO, modelo);

			RespuestaCustomizada<Iterable<ReporteEmpleados>> respuesta = new RespuestaCustomizada<>();
	...
	public HttpEntity<ByteArrayResource> download(String CLUSUARIO, String q) {
		try {
			...
			header.set(HttpHeaders.CONTENT_DISPOSITION,
					"attachment; filename=empleados_" + dateFormat.format(fecha)
							+ ".xlsx");
