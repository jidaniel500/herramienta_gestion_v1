...
import com.claro.gestionrecursosapi.reporte.domain.ReporteInfoCompromisosService;
...
@RestController
@RequestMapping("/api/v1/reporte/compromisos")
public class ReporteInfoCompromisosController {

	@Autowired
	private ReporteInfoCompromisosService service;
	...
	public ResponseEntity<?> informe(
		...
		try {
			...
			Iterable<ReporteInfoCompromisos> resultado = service.generarReporte(
					CLUSUARIO, modelo);

			RespuestaCustomizada<Iterable<ReporteInfoCompromisos>> respuesta = new RespuestaCustomizada<>();
	...
	public HttpEntity<ByteArrayResource> download(String CLUSUARIO, String q) {
		try {
			...
			header.set(HttpHeaders.CONTENT_DISPOSITION,
					"attachment; filename=compromisos_" + dateFormat.format(fecha)
							+ ".xlsx");
