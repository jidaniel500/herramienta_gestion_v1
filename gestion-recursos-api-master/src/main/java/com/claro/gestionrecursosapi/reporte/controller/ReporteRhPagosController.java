...
@RestController
@RequestMapping("/api/v1/reporte/pagos")
public class ReporteRhPagosController {

	@Autowired
	private ReporteRhPagosService service;
	...
	public ResponseEntity<?> informe(
		...
		try {
			...
			Iterable<ReporteRhPagos> resultado = service.generarReporte(
					CLUSUARIO, modelo);

			RespuestaCustomizada<Iterable<ReporteRhPagos>> respuesta = new RespuestaCustomizada<>();
	...
	public HttpEntity<ByteArrayResource> download(String CLUSUARIO, String q) {
		try {
			...
			header.set(HttpHeaders.CONTENT_DISPOSITION,
					"attachment; filename=pagos_" + dateFormat.format(fecha)
							+ ".xlsx");
