...
import com.claro.gestionrecursosapi.tarea.domain.TareaEstadoTipoService;
import com.claro.gestionrecursosapi.tarea.entity.TareaEstadoTipoEntity;

@RestController
@RequestMapping("/api/v1/tareaestadotipo")
public class TareaEstadoTipoController {

	@Autowired
	private TareaEstadoTipoService service;

	@GetMapping
	public ResponseEntity<RespuestaBase> buscarTodo() {
		try {
			Iterable<TareaEstadoTipoEntity> listasPersonas = service.findAll();
			RespuestaCustomizada<Iterable<TareaEstadoTipoEntity>> respuesta = new RespuestaCustomizada<>();
	...
	public ResponseEntity<RespuestaBase> buscarPorId(@PathVariable Integer id) {
		try {
			Optional<TareaEstadoTipoEntity> personaEntity = service.findById(id);
			RespuestaCustomizada<TareaEstadoTipoEntity> respuesta = new RespuestaCustomizada<>();
			respuesta.setCodigoEstatus(HttpStatus.OK.value());
			respuesta.setMensaje("Consulta exitosa");
	...
	@PostMapping
	public ResponseEntity<RespuestaBase> crear(@RequestBody TareaEstadoTipoEntity entity) {
		try {
			TareaEstadoTipoEntity personaEntity = service.save(entity);
	...
	@PutMapping("/{id}")
	public ResponseEntity<RespuestaBase> actualizar(@PathVariable Integer id, @RequestBody TareaEstadoTipoEntity entity) {
		try {
			TareaEstadoTipoEntity personaEntity = service.save(entity);
			RespuestaCustomizada<TareaEstadoTipoEntity> respuesta = new RespuestaCustomizada<>();
			respuesta.setCodigoEstatus(HttpStatus.OK.value());
